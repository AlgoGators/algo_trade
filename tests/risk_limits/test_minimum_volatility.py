...
import numpy as np
import pandas as pd
from algo_trade.risk_limits import minimum_volatility


class TestMinimumVolatility(unittest.TestCase):
    def test_minimum_volatility(self):
        variances = pd.read_parquet(
            "tests/risk_limits/data/GARCH_variances.parquet"
        )
        ...
        result: pd.Series = minimum_volatility(
            max_forecast_ratio,
            IDM,
            tau,
            maximum_leverage,
            1 / 3,
            annualized_volatilies.iloc[5000],
        )
