...
class Manager:
    ...
    def get_all_trend_tables(self) -> dict:
        with self.trend_engine.connect() as conn:
            return {
                ...
                for table in self.trend_engine.table_names()
            }
    ...
    def get_all_carry_tables(self) -> dict:
        with self.carry_engine.connect() as conn:
            return {
                table: pd.read_sql(f"SELECT * FROM {table}", conn)
                for table in self.carry_engine.table_names()
            }
    ...
    def create_trend_table(self, symbol: str):
        sa.Table(
            ...
            sa.Column("Backadjusted", sa.Float),
        ).create(self.trend_engine)

    def fetch_existing_dates(self, symbol: str):
        session = self.trend_session()
        existing_dates_query = session.query(sa.column("Date")).from_table(
            sa.table(f"{symbol}_data")
        )
