...
from algo_trade.ib_utils.src._enums import AccountSummaryTag
from algo_trade.ib_utils.src._contract import Contract
from algo_trade.ib_utils.src._config import TIMEOUT, WAIT_FOR_TRADES, WAIT_FOR_TRADES_TIMEOUT
from algo_trade.ib_utils.src._type_hints import ContractDetails
...
class APIHandler:
    ...
    def current_positions(self) -> list[Position]:
        ...
        for conId, (_, quantity) in positions.items():
            ...
            positions.append(
                Position(
                    contract=self.get_contract_details(contract_by_ID)[conId],
                    quantity=quantity
                )
            )
