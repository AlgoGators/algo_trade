...
from algo_trade.risk_measures import GARCH, RiskMeasure, Simple, JPMorgan
from algo_trade.dyn_opt import dyn_opt
from algo_trade.risk_limits import portfolio_multiplier, position_limit
from tests.test_dyn_opt import mockedPositionLimit, mockedPortfolioMultiplier
    ...
    def __init__(self, instruments : list[Instrument], risk_target : float, capital : float):
        ...
        self.portfolio_rules = [
            partial(
                ...
                position_limit_fn=mockedPositionLimit(
                    ...
                    IDM=2.5,
                    tau=risk_target),
                portfolio_multiplier_fn=mockedPortfolioMultiplier(
...
def main():
    futures : list[Future] = [
        ...
        Future(symbol="NQ", dataset="CME", multiplier=2),
        Future(symbol="RTY", dataset="CME", multiplier=5)
    ]

    trend: Trend = Trend(futures, 0.2, 100_000)

    # print(trend.positions.to_csv('no_cost_opt.csv'))
